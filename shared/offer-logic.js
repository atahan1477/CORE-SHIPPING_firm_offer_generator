...
export function buildOfferText(data) {
  ...
  const lines = [
    ...
    trimmed(data.productGroup) ? `Product group: ${trimmed(data.productGroup)}` : '',
    `Cargo: ${trimmed(data.cargo)}`,
    trimmed(data.cargoQuantity) ? `Qty: ${trimmed(data.cargoQuantity)}` : '',
    `${trimmed(data.underDeck)}`,
    `${trimmed(data.cargoStackable)}`,
    `${trimmed(data.pcBasis || 'P/c basis')}`,
    trimmed(data.offerDate) ? `Offer date: ${trimmed(data.offerDate)}` : '',
    trimmed(data.validityDeadline) ? `Validity: ${trimmed(data.validityDeadline)}` : '',
